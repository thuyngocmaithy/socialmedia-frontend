...
function ConversationCard({ handleChange, chattingWith }) {
    ...
    return (
        <div className={cx('wrapper-conversation-card')}
            ...
            onClick={() => handleChange(chattingWith.user.id)}
        >
            <Image src={chattingWith.user.avatar} className={cx('conversation-avatar')}></Image>
            <div className={cx('wrapper-conversation')}>
                <h3 className={cx('sender-name')}>{chattingWith.user.fullname}</h3>
                <h3 className={cx('lastMessage')}>{chattingWith.lastMessage}</h3>
            </div>
